...
export default function LichTrinhChiTiet({ data, showDate = false, onRefresh }) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (result.success) {
                // Đóng menu trước cho mượt
                setSelectedMenuIndex(null);
                setIsDeleteModalOpen(false);
                
                // Đợi 400ms để Google Sheet cập nhật xong DB rồi mới gọi load lại
                if (onRefresh) {
                    setTimeout(() => {
                        onRefresh();
                    }, 400);
    ...
    return (
        <div className="flex flex-col gap-2.5 p-1 relative">
            {data.map((item, index) => (
                <div key={index} className={`${defaultColors[index % 4]} rounded-xl p-3 relative shadow-md animate-pop-in`}>
                    <div className="absolute top-3 right-3 bottom-3 flex flex-col items-end justify-between z-20">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setSelectedMenuIndex(index); }} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/40 active:scale-75 transition-all"><svg className="w-4 h-4 text-slate-700" fill="currentColor" viewBox="0 0 24 24"><path d="M12 10c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0-6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 12c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z" /></svg></button>
                    </div>
                    <div className="text-white">
                        <div className="flex items-center gap-1"><span className="text-[9px] font-black bg-white/20 px-1 rounded">{item.loaiXe || item.label}</span><span className="text-[13px] font-bold opacity-90">• {item.taiXe || "Chưa có tài"}</span></div>
                        <div className="mt-2 flex items-center gap-2"><h3 className="text-[15px] font-bold">{item.tenKhachHang || item.tenKhach}</h3><span className="text-[12px] opacity-80">• {item.soDienThoai || item.sdt}</span></div>
                        <p className="text-[10px] italic opacity-70 mt-1 truncate">{item.ghiChu || "Không có ghi chú"}</p>
                    </div>
                </div>
            ))}
            ...
            {typeof selectedMenuIndex === 'number' && isClient && createPortal(
                <div className="fixed inset-0 z-[99999] flex items-center justify-center px-6 bg-slate-900/40 backdrop-blur-sm" onClick={() => setSelectedMenuIndex(null)}>
                    <div className="bg-white rounded-[32px] p-6 w-full max-w-[320px] shadow-2xl animate-pop-in" onClick={(e) => e.stopPropagation()}>
                        ...
                        {checkIsLocked(data[selectedMenuIndex]) ? (
                            <div className="text-center">
                                <div className="bg-red-50 p-4 rounded-2xl border border-red-100 text-red-600 font-bold text-sm mb-4">🔒 Không thể sửa chuyến đi cũ</div>
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="grid grid-cols-2 gap-3">
                                    <button className="bg-blue-500 text-white font-black py-4 rounded-2xl shadow-md">Sửa</button>
                                    <button onClick={() => setIsDeleteModalOpen(true)} className="bg-red-500 text-white font-black py-4 rounded-2xl shadow-md">Xóa</button>
                                </div>
                                <button className="bg-slate-100 py-4 rounded-2xl font-black" onClick={() => setSelectedMenuIndex(null)}>Hủy bỏ</button>
                            </div>
                        )}
                    </div>
                </div>, document.body
            ...
            {/* MODAL XÁC NHẬN XÓA */}
            {isDeleteModalOpen && isClient && createPortal(
                <ModalXoa 
                    isOpen={isDeleteModalOpen}
                    onClose={() => setIsDeleteModalOpen(false)}
                    onConfirm={confirmDelete}
                    isLoading={isDeleting}
                    itemName={data[selectedMenuIndex]?.tenKhachHang || data[selectedMenuIndex]?.tenKhach}
                />, document.body
            )}
        </div>
    );
