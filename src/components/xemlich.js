...
export default function XemLich() {
  ...
  return (
    ...
    >
      ...
      {/* 1. THANH LỊCH NGANG */}
      <motion.div variants={itemVariants} className="mx-[-17] mt-4 bg-gray-50 rounded-2xl p-1.5 flex items-center justify-between border border-gray-100 shadow-sm relative z-20">
        {/* NÚT MŨI TÊN TRÁI */}
        <button 
          onClick={() => setCurrentWeekAnchor(new Date(currentWeekAnchor.setDate(currentWeekAnchor.getDate() - 7)))} 
          className="w-6.5 h-10 bg-white border border-gray-200 shadow-sm rounded-xl flex items-center justify-center text-gray-500 z-30 relative active:scale-90 active:bg-gray-50 transition-all"
        >
      ...
      <motion.div variants={itemVariants} className="mx-[-17] h-[420px] overflow-y-auto bg-gray-50/50 rounded-2xl shadow-inner border border-gray-100 no-scrollbar relative mb-2">
        {loading ? (
